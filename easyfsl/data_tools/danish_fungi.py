...
class DanishFungi(EasySet):
    def __init__(
        ...
        self.class_names = list(self.data.scientificName.unique())
    ...
    def load_specs(specs_file: Path) -> DataFrame:
        ...
        class_names = list(data.scientificName.unique())
        label_mapping = {name: class_names.index(name) for name in class_names}
        return data.assign(label=lambda df: df.scientificName.map(label_mapping))
